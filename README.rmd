...
A benchmark is provided for a data set with 10 numerical columns, 10 string columns, and 10 date columns. `encapsulate` uses the little hack that allows us to use reticulate with different Python executables in the same R session (which means all data gets serialized and send over to another worker). `do_not_encapsulate` goes straight from the user's R session to reticulae.
